...
// ============================================================
// LIBRARY VIEW — all lessons across all tracks
// ============================================================
export function renderLibrary() {
  // Count progress per track
  const trackCard = (track, lessons, getTitle, kind) => {
    const total = lessons.length;
    return `
      <div class="library-track">
        <div class="library-track-head" style="border-left: 3px solid ${track.color};">
          <span class="library-track-icon" style="color: ${track.color};">${track.icon}</span>
          <div>
            <div class="library-track-name">${esc(track.name)}</div>
            <div class="library-track-meta">${total} lesson${total === 1 ? '' : 's'}</div>
          </div>
        </div>
        <div class="library-lesson-list">
          ${lessons.map((id, idx) => {
            const title = getTitle(id);
            const sub = (kind === 'core' ? CORE_LESSONS[id] : kind === 'sd' ? SD_LESSONS[id] : DSA_LESSONS[id])?.subtitle || '';
            return `
              <button class="library-lesson-item" data-action="select-lesson" data-kind="${kind}" data-id="${id}">
                <span class="library-lesson-num mono">${String(idx + 1).padStart(2, '0')}</span>
                <div class="library-lesson-text">
                  <div class="library-lesson-title">${esc(title)}</div>
                  ${sub ? `<div class="library-lesson-sub">${esc(sub)}</div>` : ''}
                </div>
  ...
  // Core tracks (new content)
  const coreSections = CORE_TRACKS.map(t =>
    trackCard(t, t.lessons, id => CORE_LESSONS[id]?.title || id, 'core')
  ).join('');

  // SD track (existing)
  const sdKeys = Object.keys(SD_LESSONS);
  const sdTrack = { id: 'sd', name: 'System Design', icon: '◇', color: '#7B9FB5' };
  const sdSection = trackCard(sdTrack, sdKeys, id => SD_LESSONS[id]?.title || id, 'sd');

  // DSA track (existing)
  const dsaKeys = Object.keys(DSA_LESSONS);
  const dsaTrack = { id: 'dsa', name: 'DSA Patterns', icon: '⌘', color: '#B888C0' };
  const dsaSection = trackCard(dsaTrack, dsaKeys, id => DSA_LESSONS[id]?.title || id, 'dsa');

  const totalLessons = CORE_TRACKS.reduce((s, t) => s + t.lessons.length, 0) + sdKeys.length + dsaKeys.length;

  return `
    <div class="view stagger">
      <h1 class="h1" style="margin-bottom: 4px;">Library<span style="color: var(--accent-amber);">.</span></h1>
      <div class="caption" style="margin-bottom: 32px;">${totalLessons} deep lessons across ${CORE_TRACKS.length + 2} tracks. Next/Prev between them.</div>
      ${coreSections}
      ${sdSection}
      ${dsaSection}
    </div>
